...
angular.module("bahmni.registration").controller("CreatePatientController", [
    ...
    function (
        ...
        var prepopulateDefaultsInFields = function () {
            ...
            _.chain(defaultsWithAnswers)
                .filter(isConcept)
                .each(setDefaultConcept)
                .value();
            _.chain(defaultsWithAnswers)
                .filter(isDateType)
                .each(setDefaultValue)
                .value();
        ...
        var createPatient = function (jumpAccepted) {
            return patientService.create($scope.patient, jumpAccepted).then(
                function (response) {
                    // ----------------------------------------------------
                    // 3. SUCCESS LOGIC: Show Message and Reload Page
                    // ----------------------------------------------------
        ...
        $scope.create = function () {
            ...
            // ----------------------------------------------------
            // 4. SAFETY FIX: Manual Date Correction
            // ----------------------------------------------------
            // If birthdate is a String (Nepali), force convert it to English Date
            // This prevents validation errors from blocking the save logic.
                ...
                try {
                    ...
                    if (parts.length === 3) {
                        ...
                        if (window.calendarFunctions) {
                            // Convert BS to AD (0-based month)
                    ...
                    console.error("Manual date conversion failed", e);
                }
            }
            // ----------------------------------------------------
        ...
        // Fallback for split-button actions

...
angular.module("bahmni.registration").controller("CreatePatientController", [
  ...
  function (
    ...
    $scope.getTranslatedPatientIdentifier = function (patientIdentifier) {
      return Bahmni.Common.Util.TranslationUtil.translateAttribute(
          patientIdentifier, 
    ...
    var prepopulateDefaultsInFields = function () {
      ...
      if (!patientInformation || !patientInformation.defaults) { return; }
      
      var defaults = patientInformation.defaults;
      var defaultVariableNames = _.keys(defaults);

      var hasDefaultAnswer = function (personAttributeType) { return _.includes(defaultVariableNames, personAttributeType.name); };
      var isConcept = function (personAttributeType) { return personAttributeType.format === "org.openmrs.Concept"; };

      var setDefaultAnswer = function (personAttributeType) { $scope.patient[personAttributeType.name] = defaults[personAttributeType.name]; };

      var setDefaultConcept = function (personAttributeType) {
        var defaultAnswer = defaults[personAttributeType.name];
        var isDefaultAnswer = function (answer) { return answer.fullySpecifiedName === defaultAnswer; };
      ...
      var isDateType = function (personAttributeType) { return personAttributeType.format === "org.openmrs.util.AttributableDate"; };
      var isDefaultValueToday = function (personAttributeType) { return defaults[personAttributeType.name].toLowerCase() === "today"; };

      var setDefaultValue = function (personAttributeType) {
        if (isDefaultValueToday(personAttributeType)) { $scope.patient[personAttributeType.name] = new Date(); } 
        else { $scope.patient[personAttributeType.name] = ""; }
      };

      var defaultsWithAnswers = _.chain(personAttributeTypes).filter(hasDefaultAnswer).each(setDefaultAnswer).value();
    ...
    var expandSectionsWithDefaultValue = function () {
      angular.forEach($rootScope.patientConfiguration && $rootScope.patientConfiguration.getPatientAttributesSections(), function (section) {
          var notNullAttribute = _.find(section && section.attributes, function (attribute) { return $scope.patient[attribute.name] !== undefined; });
    ...
    var prepopulateFields = function () {
      ...
      if (fieldsToPopulate) {
        _.each(fieldsToPopulate, function (field) {
          ...
          if (addressLevel) { $scope.patient.address[addressLevel.addressField] = $rootScope.loggedInLocation[addressLevel.addressField]; }
        });
      }
    };
    prepopulateFields();
    ...
    var getConfirmationViaNgDialog = function (config) {
      ...
      ngDialogLocalScope.yes = function () { ngDialog.close(); config.yesCallback(); };
      ngDialogLocalScope.no = function () { ngDialog.close(); };
    ...
    var createPatient = function (jumpAccepted) {
      return patientService.create($scope.patient, jumpAccepted).then(
        function (response) {
          ...
          // 1. Process the response data (get UUID etc)
          copyPatientProfileDataToScope(response);
          
          // 2. Show Saved Message
          messagingService.showMessage("info", "REGISTRATION_LABEL_SAVED");
          
          // 3. NAVIGATE TO PATIENT EDIT PAGE (Like Standard Bahmni)
          // This keeps the data on screen and allows starting OPD visit
        ...
        function (response) {
          if (response.status === 412) {
            ...
            getConfirmationViaNgDialog({
              ...
              data: data,
              scope: $scope,
              yesCallback: function () { return createPatient(true); },
            });
          }
    ...
    var createPromise = function () {
      ...
      createPatient().finally(function () { return deferred.resolve({}); });
    ...
    $scope.create = function () {
      ...
      // Safety: Manual Date Correction (Nepali -> English Date Object)
      if ($scope.patient.birthdate && typeof $scope.patient.birthdate === 'string') {
        try {
            var val = $scope.patient.birthdate;
            var dict = { '०':'0','१':'1','२':'2','३':'3','४':'4','५':'5','६':'6','७':'7','८':'8','९':'9' };
            var engVal = val.replace(/[०-९]/g, function (c) { return dict[c]; });
            var parts = engVal.split(/[-/.]/);
            
            if (parts.length === 3) {
                 var y = parseInt(parts[0]), m = parseInt(parts[1]), d = parseInt(parts[2]);
                 if (window.calendarFunctions) {
                    $scope.patient.birthdate = window.calendarFunctions.getAdDateByBsDate(y, m - 1, d);
                 } else if (window.NepaliFunctions) {
                    var ad = window.NepaliFunctions.BS2AD({ year: y, month: m, day: d });
                    $scope.patient.birthdate = new Date(ad.year, ad.month - 1, ad.day);
                 }
                 if ($scope.patient.birthdate instanceof Date) {
                    $scope.patient.birthdate.setHours(0,0,0,0);
                 }
            }
        } catch(e) {}
      }
      ...
      if (errorMessages.length > 0) {
        errorMessages.forEach(function (errorMessage) { messagingService.showMessage("error", errorMessage); });
        return $q.when({});
      }
    ...
    // Standard Bahmni "After Save" action
